...
local files: table = {'guis/new.lua', 'guis/rise.lua', 'init.lua', 'main.lua', 'games/universal.lua', 'games/6872274481.lua', 'games/6872265039.lua', 'libraries/base64.lua', 'libraries/construct.lua', 'libraries/prediction.lua', 'libraries/hash.lua', 'libraries/entity.lua', 'libraries/spotify.lua', 'libraries/whitelist.lua', 'libraries/login.lua'};
...
local initiate: () -> () = function(profiles: boolean): () -> ()
    ...
    if isfolder('newcatvape') then
        for i: number, v: string in listfiles('newcatvape/') do
            if v ~= 'newcatvape/Profiles' then
    ...
    for i: number, v: string in {'assets', 'games', 'libraries', 'profiles', 'guis'} do
        installtext.Text = `Installing newcatvape/{v}`
        makefolder(`newcatvape/{v}`);
        installtext.Text = `Installed newcatvape/{v}`;
    end;
    stage = 400;
    for i: number, v: string in files do
        ...
        if file ~= '404: Not Found' then
            writefile(`newcatvape/{v}`, file);
            installtext.Text = `Installed newcatvape/{v}`;
        else
            installtext.Text = `Failed to install newcatvape/{v}`;
            warn(`❌ [ Installer Error ]: Failed to install, file named newcatvape/{v} doesn't exist on the github.`);
        end;
    end;
    stage = 200;
    for i: number, v: table in httpservice:JSONDecode(game:HttpGet('https://api.github.com/repos/qwertyui-is-back/CatV5/contents/assets')) do
        if v.name then
            installtext.Text = `Installing newcatvape/assets/{v.name}`;
            if not isfile(`newcatvape/assets/{v.name}`) then
                writefile(`newcatvape/assets/{v.name}`, game:HttpGet('https://raw.githubusercontent.com/qwertyui-is-back/CatV5/refs/heads/main/assets/'..v.name));
            else
                task.wait(0.05);
            end;
            stage += 0.4;
            installtext.Text = `Installed newcatvape/assets/{v.name}`;
        end;
    end;
    fakefinished = true
