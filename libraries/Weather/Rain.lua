...
local RAIN_STRAIGHT_MAX_SPEED = 70							-- Maximum speed for the straight rain emitter
...
function Rain:SetStraightSpeed(asset)
	
	if typeof(asset) ~= "number" then
		error("bad argument #1 to 'SetStraightSpeed' (number expected, got " .. typeof(asset) .. ")", 2)
	end
	
	RAIN_STRAIGHT_MAX_SPEED = asset
	for _,v in pairs(rainAttachments) do
		v.RainStraight.Speed = NumberRange.new(asset)
	end
end

function Rain:SetStraightSize(asset)
	
	if typeof(asset) ~= "number" then
		error("bad argument #1 to 'SetStraightSpeed' (number expected, got " .. typeof(asset) .. ")", 2)
	end
	
	RAIN_STRAIGHT_SIZE = NumberSequence.new(asset)
	for _,v in pairs(rainAttachments) do
		v.RainStraight.Size = NumberSequence.new(asset)
	end
end
