...
local function getItemFromChest(name, priocheck, chest)
	local itemData = bedwars.ItemMeta[name]

	local fchest, item

	if chest then
		for i,v in chest:GetChildren() do
			if v.Name == name then
				return true
			end
		end
		...
		for _, chest in collectionService:GetTagged('chest') do
			for __, v in chest.ChestFolderValue.Value:GetChildren() do
				if v.Name == name then
					fchest = chest
					item = v
				elseif priocheck and (itemData.sword or itemData.armor) then
					local chestItem = bedwars.ItemMeta[v.Name]
					local index, selfindex = chestItem and (chestItem.sword or chestItem.armor), item and (bedwars.ItemMeta[item.Name] and (bedwars.ItemMeta[item.Name].sword or bedwars.ItemMeta[item.Name].armor)) or (itemData.sword or itemData.armor)
					
					index = index and (rawget(index, 'damageReductionMultiplier') or rawget(index, 'damage'))
					selfindex = selfindex and (rawget(selfindex, 'damageReductionMultiplier') or rawget(selfindex, 'damage'))
					
					if index and index > selfindex then
						fchest = chest
						item = v
					end
				end
...
run(function()
	...
	local remoteNames = {
		...
		DepositPinata = identifyexecutor() == 'Delta' and run or debug.getproto(debug.getproto(Knit.Controllers.PiggyBankController.KnitStart, 2), 5),
		...
		HarvestCrop = identifyexecutor() == 'Delta' and run or debug.getproto(debug.getproto(Knit.Controllers.CropController.KnitStart, 4), 1),
	...
	bedwars.breakBlock = function(block, effects, anim, customHealthbar)
		if lplr:GetAttribute('DenyBlockBreak') or not entitylib.isAlive or FlyLandTick > tick() then return end
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if vape.Modules.Fly.Enabled or FlyLandTick > tick() or vape.Modules['Long Jump'].Enabled then
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and tick() > FlyLandTick and isnetworkowner(entitylib.character.RootPart) then
...
local Attacking
local projfinished = true
getgenv().projectileCount = {}
local Registering
local ProjectileAura = {}
getgenv().canProj = 0
local LastSwing = workspace:GetServerTimeNow()
run(function()
	local Killaura
	local MultiAura
	local Targets
	...
	local Limit
	local LegitAura = {}
	local Particles, Boxes, TargetTable = {}, {}, {}
				...
				repeat
					...
					if sword then
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								if not projfinished or canProj > tick() then
									Registering = false
									task.wait()
									break
								end
								if lplr.Character:FindFirstChild('HandInvItem') and lplr.Character.HandInvItem.Value ~= sword.tool then
									task.spawn(switchItem, sword.tool, 0)
								end
								...
								if actualRoot then
									...
									if (vape.Modules['Auto Dodge'].Enabled and AntiHitOnGround or not vape.Modules['Auto Dodge'].Enabled) and tick() > swingCooldown then
										swingCooldown = tick() + (ProjectileAura.Enabled and 0.22 or 0.1)
									...
									if #projectileCount > 0 then
										if HitCount > (#projectileCount > 2 and 80 or #projectileCount > 1 and 100 or 120) then
											HitCount = 0
											if ProjectileAura.Enabled and MultiAura.Enabled then
												getgenv().canProj = tick() + 0.2
												task.wait(0.02)
												break
											end
										end
		...
		Function = function(callback)
			...
			if callback then
				Speed:Clean(runService.PreSimulation:Connect(function(dt)
					...
					if entitylib.isAlive and not Fly.Enabled and tick() > FlyLandTick and not LongJump.Enabled and isnetworkowner(entitylib.character.RootPart) and not pingSpiking then
...
run(function()
	...
	List = StorageESP:CreateTextList({
		Name = 'Item',
		Function = function()
			storageesplist = List.ListEnabled
			for _, v in Reference do
	...
	local function kitCollection(id, func, range, specific)
		...
		repeat
			if entitylib.isAlive then
				...
				for _, v in objs do
					if FlyLandTick > tick() or not AutoKit.Enabled then break end
				...
				repeat
					if entitylib.isAlive then
						...
						for _, v in items do
							...
							if PickItems[v] and PickItems[v] > tick() then continue end
...
run(function()
	...
	local function staffFunction(plr, checktype)
		...
		elseif Mode.Value == 'AutoConfig' then
			local safe = {'Auto Clicker', 'Reach', 'Sprint', 'Hit Fix', 'Staff Detector'}
			vape.Save = function() end
	...
	Mode = StaffDetector:CreateDropdown({
		Name = 'Mode',
		List = {'Uninject', 'Profile', 'AutoConfig', 'Requeue', 'Notify', 'Lobby'},
		Function = function(val)
...
run(function()
	...
	local function lootChest(chest)
		chest = chest and chest.Value or nil
		local chestitems = chest and chest:GetChildren() or {}
		if #chestitems > 0 and (Delays[chest] == nil or Delays[chest] < tick()) then
			Delays[chest] = tick() + 0.4
