...
local knit = game:GetService("ReplicatedStorage").Modules.Knit
    ...
    Killaura = vape.windows.blatant.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
                ...
                table.insert(Killaura.Connections, lplr.CharacterAdded:Connect(function(c)
                    repeat task.wait() until c.Humanoid ~= nil
                    updateViewmodel()
                    task.spawn(function()
                        local oldNearPlayer
                        repeat
			...
			if callback then
				...
				BindToRenderStep("Speed", 1, function(delta)
					if entityLibrary.isAlive and (isnetworkowner(lplr.Character.HumanoidRootPart) or acben) then
                        boostDelay += 1
                        local literalSpeed = SpeedValue.Value
						...
						if SpeedMethod.Value == "Boost" then
							...
							elseif boostDelay >= 12 then
								boostedSpeed = 0
								lplr.Character.Humanoid.WalkSpeed = literalSpeed
							end
						else
							boostedSpeed = 0
							lplr.Character.Humanoid.WalkSpeed = literalSpeed
        ...
        Function = function(callback)
            if callback then
                ...
                BindToStepped("fly",1,function()
                    if FlyUp then
                        YCFrame += VerticalSpeed.Value / 18.5
                    elseif FlyDown then
                        YCFrame -= VerticalSpeed.Value / 18.5
                    end
...
run(function()
    ...
    local old
    local oldwarn
    SecurityFeatures = vape.windows.utlity.CreateOptionsButton({
