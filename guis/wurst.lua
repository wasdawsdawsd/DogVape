...
local getcustomassets = {
	['DogVape/assets/wurst/triangle.png'] = 'rbxasset://wurst/triangle.png'
}
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/new-qwertyui/CatV5/'..readfile('DogVape/profiles/commit.txt')..'/'..select(1, path:gsub('DogVape/', '')), true) end)
...
function mainapi:CreateCategory(categorysettings)
	...
	function categoryapi:CreateModule(modulesettings)
		...
		triangle.Image = getcustomasset('DogVape/assets/wurst/triangle.png')
